...
uint8_t send_log[16]={100};//for tap looper
uint8_t log_sensor_num=15;//for tap looper
...
void __attribute__((interrupt, auto_psv)) _U1RXInterrupt(void) {
    ...
    while (U1STAbits.URXDA == 1) {
        ...
        if(buff_2 == 2){
            ...
            if((main_mode == MODE_PRESSURE) || (main_mode == MODE_TAP)){
...
void __attribute__((interrupt, auto_psv)) _DMA1Interrupt(void) {
    ...
    if (meas_count == 1) {
        ...
        */
        ...
        if (main_mode == MODE_TAP && serial_flg == 1) {
            
            for (k = 0; k < 16; k++) {
                if(log_sensor_num!=sensor_num)
                        send_log[k]=0;
//                
//                if(looper[7] == 1 && clk_1_4_on == 1 && note_value_clk[k][sensor_num] != 0 && send_log [k]==0){
//                    send_data[k * 2] = 0x0f & (uint8_t) (note_value_clk[k][sensor_num] >> 6);
//                    send_data[k * 2 + 1] = 0x40 | (uint8_t) (note_value_clk[k][sensor_num] & 0x003f);
//                    send_log[k]=1;
//                }
//                else if(looper[7] == 1 && clk_1_4_on == 1 && send_note_log[k][timing_log] != 0 && send_log [k]==0){
//                    send_data[k * 2] = 0x0f & (uint8_t) (send_note_log[k][timing_log] >> 6);
//                    send_data[k * 2 + 1] = 0x40 | (uint8_t) (send_note_log[k][timing_log] & 0x003f);                    
//                    send_log[k]=1;
//                }
//                else if(looper[7] == 1 ){
//                    send_data[k * 2] = 0x00;
